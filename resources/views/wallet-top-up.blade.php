...
<head>
  ...
  <title>Unka Go Wallet Top Up</title>
...
<li class="nav-item"><a class="nav-link" href="{{ route('about') }}">About</a></li>
  ...
  <div class="form-wrapper">
    <div class="purchase-form-container">
      ...
      <form id="floatPurchaseForm" method="POST" action="{{ route('topup.customer') }}">
        @csrf
        <div class="form-group">
          ...
          <input type="text" class="form-control" name="registeredNumber" id="registeredNumber"
            placeholder="Enter registered number (e.g. 0980888111)" required>
        </div>
        ...
        <div class="payment-options text-center mt-3">
          ...
          <img src="{{ asset('images/zamtel.png') }}" alt="Zamtel">
          <img src="{{ asset('images/zedmobile.png') }}" alt="ZedWallet">
        </div>

        <button type="submit" class="btn btn-purchase mt-3">
          <i class="fas fa-shopping-cart mr-2"></i> Complete Top Up
        </button>
      </form>
    </div>
  </div>
            ...
            if (response.success) {
              $('#messageContainer').html(`
                <div class="alert alert-success">
                  ✅ ${response.message || 'Top-up successful!'}<br>
                </div>
              `);
              form[0].reset();
              updateSummary();
              ...
              $('#messageContainer').html(`
                <div class="alert alert-danger">❌ ${response.error || 'Top-up failed.'}</div>
              `);
            }
          })
          .fail(function (xhr) {
